use rdkafka::{
    consumer::{Consumer, StreamConsumer}, config::ClientConfig,
...
async fn main() {
    ...
    let consumer: StreamConsumer = ClientConfig::new()
        .set("bootstrap.servesrs", "localhost:9092")
        .set("group_id", "rust_log_consumer")
...
fn handle_message(msg: BorrowedMessage) {
    if let Some(Ok(payload)) = msg.payload_view() {
        match serde_json::from_str::<LogMessage>(payload) {
            Ok(log_message) => info!("Consumed log: {:?}", log_message),
            Err(e) => error!("Failed to parse JSON: {}", e),
        }
    } else {
        error!("Empty/Unreadable Kafka log message");
    }
}
